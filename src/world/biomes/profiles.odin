...
biome_profile_for :: proc(biome_id: BiomeID) -> BiomeProfile {
	...
	case .Wet_Lowland_Marsh:
		return {
			biome_id = biome_id,
			base_height_blocks = 32,
			continental_height_blocks = 34,
			elevation_height_blocks = 22,
			erosion_height_blocks = -2,
			relief_height_blocks = 7,
			...
			shoreline_width_blocks = 12,
			shoreline_slope = 0.48,
			underwater_floor_depression_blocks = 0.5,
			cliff_coast_bias = 0.00,
			swamp_shallowness = 0.25,
	...
	case .Corrupted_Fen:
		return {
			biome_id = biome_id,
			base_height_blocks = 34,
			continental_height_blocks = 38,
			elevation_height_blocks = 26,
			erosion_height_blocks = -2,
